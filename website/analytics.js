(() => {
  ...
  if (window.location.pathname === '/pricing/' || window.location.pathname === '/pricing/index.html') {
    track('pricing_view');
  }
  ...
  if (pricingSection && 'IntersectionObserver' in window) {
    const pricingObserver = new IntersectionObserver((entries) => {
      ...
      track('pricing_view', { pricing_surface: 'homepage' });
  ...
  document.addEventListener('click', (event) => {
    ...
    if (destination.hostname === 'forms.gle') {
      track('waitlist_click');
      return;
    }

    if (/(contact us|talk to sales|evaluate for teams)/.test(label)) {
